...
//    wire sel = (S_AXIS_TDATA[7 : 0] <= 8'd100) && (S_AXIS_TDATA[15: 8] >= 8'd200) && (S_AXIS_TDATA[23: 16] <= 8'd100);
    ...
    wire [bit_num_x:0] x_loc;
    reg [bit_num_y:0] y_loc;
    
    assign x_loc = write_pointer % H_RESOLUTION ;
    ...
    always @(posedge S_AXIS_ACLK) begin
        if(!S_AXIS_ARESETN) begin
           ...
           y_center <= 0;
            ...
            if(write_state == DETECTION && x_loc == H_RESOLUTION - 1) begin
    ...
    always @(posedge S_AXIS_ACLK)
       begin
           ...
           begin
               ...
               if (write_state != DETECTION && y_loc == 0) begin
                   x1 <= 0;
                   x2 <= 0;
                   y1 <= 0;
                   y2 <= 0;               
                   ...
                   if (current_is_red && !temp_is_red) begin
                       x1 <= x_loc;
                       y2 <= y_loc; 
                       if (!y1)
                           y1 <= y_loc;                
                   end
                   else if (!current_is_red && temp_is_red) begin
                       x2 <= x_loc - 1;
                   end
               end
           end
