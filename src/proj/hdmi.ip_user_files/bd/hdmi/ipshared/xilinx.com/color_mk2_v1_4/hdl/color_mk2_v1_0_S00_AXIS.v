...
//    wire sel = (S_AXIS_TDATA[7 : 0] <= 8'd100) && (S_AXIS_TDATA[15: 8] >= 8'd200) && (S_AXIS_TDATA[23: 16] <= 8'd100);
    ...
    always @(posedge S_AXIS_ACLK)
       begin
           ...
           else if(write_state == DETECTION)
           begin
               pixel_temp <= pixel_current;
               pixel_current <= S_AXIS_TDATA;
           end
           if (pixel_current[23:16] > 8'd200)
           begin
               if(pixel_temp[23:16] <= 8'd200)
               begin
                   x1 <= write_pointer;
                   y2 <= y_loc; 
                   if (!y1 && y_loc != NUMBER_OF_INPUT_WORDS_Y)
                       y1 <= y_loc;
                   end                   
           end
           else if (pixel_current[23:16] <= 8'd200)
           begin
               if(pixel_temp[23:16]> 8'd200)
                   x2 <= write_pointer - 1;
           end
           else if (y_loc == NUMBER_OF_INPUT_WORDS_Y) begin
               x1 <= 0;
               x2 <= 0;
               y1 <= 0;
               y2 <= 0;               
       ...
       assign x_center = (x1 + x2) / 2;
       assign y_center = (y1 + y2) /2; 
