...
`timescale 1 ns / 1 ps
	...
	(
        ...
        parameter integer NUMBER_OF_INPUT_WORDS_X  = 1920,
        parameter integer NUMBER_OF_INPUT_WORDS_Y = 1080, 
	...
	(
        ...
        input wire [bit_num_y-1:0] y_loc,
        
        
        output reg [bit_num_x-1:0] x1, x2,
        output reg [bit_num_y-1:0] y1, y2,
        
        output wire [bit_num_x-1:0] x_center,
        output wire [bit_num_y-1:0] y_center,
        
        output reg [C_S_AXIS_TDATA_WIDTH-1 : 0] pixel_current,
        output reg [C_S_AXIS_TDATA_WIDTH-1:0] pixel_temp,
        // User ports ends
...
//	localparam NUMBER_OF_INPUT_WORDS  = 8;
	...
	localparam bit_num_x  = clogb2(NUMBER_OF_INPUT_WORDS_X-1);
	localparam bit_num_y  = clogb2(NUMBER_OF_INPUT_WORDS_Y-1);
	...
	assign axis_tready = ((mst_exec_state == WRITE_FIFO) && (write_pointer <= NUMBER_OF_INPUT_WORDS_X-1));
	...
	begin
	  if(!S_AXIS_ARESETN)
	    ...
	    if (write_pointer <= NUMBER_OF_INPUT_WORDS_X-1)
	      begin
	        if (fifo_wren)
	          begin
...
//	            writes_done <= 1'b0;
	          ...
	          if ((write_pointer == NUMBER_OF_INPUT_WORDS_X-1)|| S_AXIS_TLAST)
	            begin
...
//    wire sel = (S_AXIS_TDATA[7 : 0] <= 8'd100) && (S_AXIS_TDATA[15: 8] >= 8'd200) && (S_AXIS_TDATA[23: 16] <= 8'd100);
      ...
      if (!S_AXIS_ARESETN) 
        ...
        case (write_state)
          ...
          REPLACEMENT: 
           ...
           STREAM: 
              if (read_pointer == NUMBER_OF_INPUT_WORDS_X)
                begin
                  write_state <= DETECTION;
                  FIFO_VALID <= 0;
                end
    ...
    // Laser Detection
    wire current_is_red = (pixel_current[23:16] > ((pixel_current[7 : 0] + pixel_current[15:8])/2)) && ((pixel_current[23:16] - ((pixel_current[7 : 0] + pixel_current[15:8])/2)) >= 8'd100);
    wire temp_is_red = (pixel_temp[23:16] > ((pixel_temp[7 : 0] + pixel_temp[15:8])/2)) && ((pixel_temp[23:16] - ((pixel_temp[7 : 0] + pixel_temp[15:8])/2)) >= 8'd100);
